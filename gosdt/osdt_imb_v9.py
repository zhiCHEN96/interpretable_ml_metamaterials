...
class Objective: 
    ...
    def leaf_predict(self, p, n, w=None):
        ...
        if self.name == 'acc':
            ...
            if w*p <= n:
                predict = 0
        elif self.name == 'precision_recall':
            if w*p <= n:
                predict = 0 
        return predict
    ...
    def loss(self, FP, FN, w=None, leaves=None, theta=None):
        if self.name == 'acc':
            ...
            loss = self.precision_loss(FP, FN)
        elif self.name == 'precision_recall':
            loss = self.precision_recall_loss(FP, FN)
    ...
    def incre_accu_bound(self, removed_l, new_l1, new_l2, unchanged_leaves=None, 
        ...
        if self.name == 'acc':
            ...
            a = self.precision_loss(FPu+removed_l.fp, FNu+removed_l.fn) - \
                self.precision_loss(FPu+new_l1.fp+new_l2.fp, FNu+new_l1.fn+new_l2.fn)
        elif self.name == 'precision_recall':
            a = self.precision_recall_loss(FPu+removed_l.fp, FNu+removed_l.fn) - \
                self.precision_recall_loss(FPu+new_l1.fp+new_l2.fp, FNu+new_l1.fn+new_l2.fn)
        elif self.name == 'auc_convex':
    ...
    def leaf_support_bound(self, leaf, w=None, theta=None, 
        ...
        if self.name == 'acc':
            ...
            tau = ld - self.precision_loss(FP-leaf.fp, FN-leaf.fn)
        elif self.name == 'precision_recall':
            tau = ld - self.precision_recall_loss(FP-leaf.fp, FN-leaf.fn)
        elif self.name == 'auc_convex':
    ...
    def precision_loss(self, FP, FN):
        return (FP+1e-3)/(self.P+FP-FN+1e-3) + 25/(self.P-FN+1e-3)
    
    def precision_recall_loss(self, FP, FN):
        alpha = 0.1
        return alpha * (FP+1e-3)/(self.P+FP-FN+1e-3) + (1-alpha) * FN / self.P
...
def get_cannot_split(name, P, N, lamb, m, new_tree_leaves, MAXDEPTH, w, theta):
    ...
    if name == 'precision':
        FP, FN = get_false(new_tree_leaves)
        ld = bound.precision_loss(FP, FN)
    if name == 'precision_recall':
        FP, FN = get_false(new_tree_leaves)
        ld = bound.precision_recall_loss(FP, FN)
    if name == 'auc_convex':
...
def predict(name, leaves_c, prediction_c, nleaves, dic, x, y, best_is_cart, clf, w=None, theta=None, logon=False):
    ...
    if best_is_cart:
        ...
        if name == 'acc':
            ...
            out = sklearn.metrics.precision_score(y, yhat)
        elif name == 'recall':
            out = sklearn.metrics.recall_score(y, yhat)
        #if (name != 'auc_convex') & (name != 'partial_auc'):
